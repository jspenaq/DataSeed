...
async def test_check_conditional_headers_if_none_match():
    ...
    # Test matching ETag
    request.headers.get.side_effect = lambda header: {"If-None-Match": 'W/"abc123"', "If-Modified-Since": None}.get(
        header,
    )
    ...
    request.headers.get.side_effect = lambda header: {"If-None-Match": 'W/"different"', "If-Modified-Since": None}.get(
        header,
    )
    ...
    # Test wildcard
    request.headers.get.side_effect = lambda header: {"If-None-Match": "*", "If-Modified-Since": None}.get(header)
...
def test_set_cache_headers():
    ...
    cache_info = CacheInfo(etag='W/"abc123"', last_modified=datetime(2024, 1, 1, 12, 0, 0, tzinfo=UTC))
...
async def test_cache_dependency_304_response():
    ...
    request.headers.get.side_effect = lambda header: {"If-None-Match": 'W/"abc123"'}.get(header)
    ...
    with (
        patch("app.api.caching.generate_request_fingerprint", return_value="request123"),
        patch(
            "app.api.caching.generate_data_fingerprint",
            return_value=("data456", datetime(2024, 1, 1, 12, 0, 0, tzinfo=UTC)),
        ),
...
async def test_cache_dependency_200_response():
    ...
    with (
        ...
        assert cache_info.last_modified == datetime(2024, 1, 1, 12, 0, 0, tzinfo=UTC)
